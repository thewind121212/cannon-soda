...
export default function Sence({}: Props) {
  const setIsReady = useStore((state) => state.isReady);
  ...
  useGSAP(() => {
    ...
    // timeline
    const introSenceTl = gsap.timeline({
      defaults: {
        duration: 3,
        ease: "back.out(1.4)",
      },
    ...
    if (window.scrollY < 20) {
      introSenceTl.from(can1GroupRef.current.position, { y: -5, x: 1 }, 0);
      introSenceTl.from(can1GroupRef.current.rotation, { z: 3 }, 0);
      introSenceTl.from(can2GroupRef.current.position, { y: 5, x: 1 }, 0);
      introSenceTl.from(can2GroupRef.current.rotation, { z: 3 }, 0);
    }

    const scrollSenceTrigger = gsap.timeline({
      defaults: {
        duration: 2,
      },
      scrollTrigger: {
